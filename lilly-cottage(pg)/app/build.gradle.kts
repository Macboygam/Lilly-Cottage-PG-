...
if (!keystoreFile.exists() && base64File.exists()) {
    try {
        val base64Content = base64File.readText()
            .replace("\\s".toRegex(), "") // remove any formatting or newlines
        val bytes = Base64.getDecoder().decode(base64Content)
        keystoreFile.writeBytes(bytes)
        logger.lifecycle("--- Decoded debug.keystore successfully from debug.keystore.base64 ---")
    } catch (e: Exception) {
        logger.error("Failed to decode debug.keystore.base64", e)
    }
}
...
android {
  ...
  defaultConfig {
    applicationId = "com.aistudio.dormitoryadmin.xpwlrz"
    minSdk = 24
    targetSdk = 34
    versionCode = 1
    versionName = "1.0"
...
tasks.register("copyApkToRoot") {
    ...
    doLast {
        ...
        if (apkFile.exists()) {
            val destFile = File(rootDir, "Dormitory_Admin_App.apk")
